Vnxcoin Core version 0.13.2 is now available from:

  <https://download.vnxcoin.org/vnxcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/vnxcoin-project/vnxcoin/issues>
...
updates will be released anymore. Without security updates, using a vnxcoin
...
ECDSA signatures inside Vnxcoin transactions now use validation using
...
Previous versions of Vnxcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Vnxcoin Core was to raise the default minimum
relay fee.

Vnxcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Vnxcoin Core 0.13.2 also introduces new default policy limits on the length and
...
Vnxcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Vnxcoin Core will relay transactions with
...
entirely in Vnxcoin Core 0.13, and it is at this time undecided whether the
...
Vnxcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Vnxcoin Core automatically creates a hidden service to listen on, without
manual configuration. Vnxcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Vnxcoin Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Vnxcoin
Core to use floating fees. By default, Vnxcoin Core will use floating
fees.
...
At all times, Vnxcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Vnxcoin Core will never create transactions paying less than
...
`vnxcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
- Adding `dbcache=100` in vnxcoin.conf
...
For example:
    ...
    $ src/vnxcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Vnxcoin Core code base has
...
- `vnxcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `vnxcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Vnxcoin scripting system that in combination with
...
Vnxcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Vnxcoin Core.
...
The two phases are distinct in the Vnxcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Vnxcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Vnxcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Vnxcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of vnxcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of vnxcoins being spent is exactly
...
- **Increased security for multisig:** Vnxcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Vnxcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Vnxcoin's current ledger (called the UTXO set),
  ...
  note that Vnxcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Vnxcoin users to individually opt-in to almost any change in the Vnxcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Vnxcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Vnxcoin
client to Vnxcoin Core 0.13.2 to signal for segwit support. When a
...
Since Vnxcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `vnxcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `vnxcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
